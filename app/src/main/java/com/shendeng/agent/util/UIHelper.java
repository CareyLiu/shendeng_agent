...
public class UIHelper {
    ...
    public final static String WEB_STYLE = linkCss + "<style>* {font-size:14px;line-height:20px;} p {color:#333;} SettingActivity_back {color:#3E62A6;} img {max-width:310px;} "
            ...
            + "SettingActivity_back.tag {font-size:15px;text-decoration:none;background-color:#bbd6f3;border-bottom:2px solid #3E6D8E;border-right:2px solid #7F9FB6;color:#284a7b;margin:2px 2px 2px 0;padding:2px 4px;white-space:nowrap;}</style>";



//    public static void showHome(Activity activity) {
//        HomeTabHostActivity.actionStart(activity);
//        activity.finish();
//    }
...
//				((App) context.getApplication()).getLoginUid());
//		intent.putExtra("friend_id", friendId);
//		intent.putExtra("friend_name", friendName);
...
//		App ac = (App) context.getApplicationContext();
//		if (!ac.isLogin()) {
...
//			public void onTextChanged(CharSequence s, int activity_start, int before,
//					int count) {
//				// 保存当前EditText正在编辑的内容
//				((App) context.getApplication()).setProperty(temlKey,
//						s.toString());
//			}
//
//			public void beforeTextChanged(CharSequence s, int activity_start, int count,
//					int after) {
//			}
//
...
//			String temlKey) {
//		String tempContent = ((App) context.getApplication())
//				.getProperty(temlKey);
...
//				builder.setSpan(span, matcher.activity_start(), matcher.end(),
...
//		if (!((App) context.getApplicationContext()).isLogin()
//				|| notice == null)
//			return;
...
//			activity_start = title.indexOf(objecttitle);
//			if (objecttitle.length() > 0 && activity_start > 0) {
//				end = activity_start + objecttitle.length();
//				sp.setSpan(new AbsoluteSizeSpan(14, true), activity_start, end,
//						Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
//				sp.setSpan(
//						new ForegroundColorSpan(Color.parseColor("#0e5986")),
//						activity_start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
//			}
//		}
//		return sp;
//	}
...
//			span = Html.fromHtml(content);
//			view.setText(span);
//			activity_start = action.length();
//			end = activity_start + name.length();
//		}
...
//
//		// 设置用户名字体加粗、高亮
//		style.setSpan(new StyleSpan(android.graphics.Typeface.BOLD), activity_start,
...
//
    ...
    public static void SnackbarMessage(View view, String msg, int time) {
        Snackbar.make(view, msg, time).setAction("Action", null).show();
    }
...
//		if (((App) activity.getApplication()).isLoadImage()) {
//			qa.setIcon(MyQuickAction.buildDrawable(activity,
...
//		App ac = (App) activity.getApplication();
//		if (ac.isLogin()) {
//			ac.Logout();
...
//					App ac = (App) context.getApplication();
//					if(!ac.isLogin()){
